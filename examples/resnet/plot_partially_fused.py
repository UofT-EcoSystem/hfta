...
def get_thoughput(args, precs, start=0, end=10, normalize=True, devide="cuda"):
  ...
  partially_fused_root = os.path.join(args.outdir, "partially_fused",
                                      args.device, args.device_model)
  ...
  for prec in precs:
    ...
    throughputs["serial:{}".format(prec)] = _calculate_throughputs([
        timing_df,
    ], devide)

    partially_fused_throughputs = []
    for i in range(start, end + 1):
      end_outdir_path = os.path.join(partially_fused_root, prec,
                                     'serial{}'.format(i))
      timing_df = pd.read_csv(os.path.join(end_outdir_path, "timing.csv"))
      partially_fused_throughputs.append(
          _calculate_throughputs([
              timing_df,
          ], devide))
    throughputs["hfta:{}".format(prec)] = np.array(partially_fused_throughputs)

  if normalize:
    std = throughputs["serial:fp32"] if "fp32" in precs else throughputs[
        "serial:amp"]
    for prec in precs:
...
def main(args):
  ...
  polt_thoughtput(throughputs, args.precs,
                  args.save_dir if args.save_dir is not None else args.outdir)
  pass
